...
void check(T result, const char *function, const char *file, size_t line) {
  if (result) {
    std::cerr << "CUDA error at " << file << ":" << line << " function "
              << function << " error code: " << cudaGetErrorName(result)
...
bool checkGroundTruth(const T *res1, const T *res2, int m) {
  for (int i = 0; i < m; ++i) {
    if (fabs(res1[i] - res2[i]) > 1e-3) {
      std::cerr << "Error at " << i << " " << std::setw(5) << std::fixed
                << res1[i] << " " << res2[i] << std::endl;
      return false;
    }
  }
  return true;
  ...
  for (int i = 0; i < VECS_PER_THREAD; ++i) {
    ...
    if (idx < n) {
      float4 mat4 = reinterpret_cast<float4 *>(
          mat)[bid * (n / VEC_SIZE) + idx]; // 1 * float4
      float4 vec4 = reinterpret_cast<float4 *>(
          vec)[idx]; // after reinterpret_cast,vec[0]:
                     // v0,v1,v2,v3;vec[1]:v4,v5,v6,v7
...
void gemv_kernel(T *mat, T *d_mat, T *vec, T *d_vec, T *dst, T *d_dst) {
  ...
  DispatchLauncher<VECS_PER_THREAD, VEC_SIZE, THREAD_NUMS>::template launch(
      d_mat, d_vec, d_dst, M, N);
...
// Cols: mat's Cols;
// get the threads needed when prcocessing mat's one row. row/4 when T is fp32,
// row/8 when T is fp16
template <size_t Cols, typename T> struct ThreadsPerMatRow {
...
__global__ void gemvKernel(float *mat, float *vec, float *dst, int N, int M) {
  ...
  // one block size can deal with rows_per_block rows
  ...
  for (int row = rows_per_block; row >= 2; row /= 2) {
    ...
    if (mat_row >= mid && mat_row < row) {
      ...
      *reinterpret_cast<float4 *>(&out_smem[(mat_row - mid) * M + mat_index]) =
          out;
    }
    __syncthreads();
    if (mat_row < mid) {
...
struct DispatchLauncher {
  ...
  static void launch(T *mat, T *vec, T *dst, int n, int m) {
    ...
    cudaEventCreate(&stop);
    cudaEventRecord(start, 0);
    gemvKernel<THREADS_PER_ROW, THREADS_PER_BlOCK, VEC_SIZE>
        <<<grid, block>>>(mat, vec, dst, n, m);
    CUDACHECK(cudaGetLastError());
    cudaEventRecord(stop, 0);
    cudaEventSynchronize(stop);
    cudaEventElapsedTime(&time, start, stop);
...
void gemv_kernel(T *mat, T *d_mat, T *vec, T *d_vec, T *dst, T *d_dst) {
  ...
  DispatchLauncher<THREADS_PER_ROW, VEC_SIZE,
                   THREAD_Per_Block>::template launch(d_mat, d_vec, d_dst, N,
                                                      M);
